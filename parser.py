...
async def parse_kwork():
    try:
        async with async_playwright() as p:
            ...
            logger.debug("Открываю страницу...")
            await page.goto("https://kwork.ru/projects", wait_until="domcontentloaded", timeout=30000)

            logger.debug("Страница загружена, жду 5 сек...")
            await asyncio.sleep(5)

            logger.debug("Получаю HTML...")
            content = await asyncio.wait_for(page.content(), timeout=15)

            logger.debug("Парсю карточки...")
            ...
            logger.debug(f"Готово, найдено {len(parsed_info)} карточек")
            await browser.close()
            return parsed_info

    except asyncio.TimeoutError:
        logger.error("parse_kwork() завис и был прерван по таймауту")
        return []
    except Exception as e:
        logger.error(f"parse_kwork() завершился с ошибкой: {e}")
        return []
